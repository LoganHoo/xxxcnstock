...
# 设置 Polars 线程数避免多线程问题（必须在 import polars 之前）
import os
...
class ConfigManager:
    ...
    def __init__(self, config_path: str):
        ...
        self.logger = get_logger("tomorrow_picks.config")
...
class DataLoader:
    ...
    def __init__(self, data_path: str, kline_dir: str = None, stock_list_path: str = None, 
        ...
        self.logger = get_logger("tomorrow_picks.data_loader")
    ...
    def load_from_kline(self) -> pl.DataFrame:
        ...
        if failed_files:
            self.logger.warning(f"跳过 {len(failed_files)} 个损坏的 parquet 文件")
...
class FilterEngine:
    ...
    def __init__(self, config_manager: ConfigManager):
        ...
        self.logger = get_logger("tomorrow_picks.filter_engine")
...
class EmailNotifier:
    ...
    def __init__(self, config: dict):
        self.config = config
        self.logger = get_logger("tomorrow_picks.notifier")
...
class MySQLStorage:
    ...
    def __init__(self, config: dict):
        self.config = config
        self.logger = get_logger("tomorrow_picks.db")
        self.engine = None
    ...
    def _init_connection(self):
        ...
        try:
            ...
            db_name = os.getenv('MYSQL_DATABASE', os.getenv('DB_NAME', 'xcn_db'))
            db_charset = os.getenv('MYSQL_CHARSET', os.getenv('DB_CHARSET', 'utf8mb4'))
...
class StockRecommender:
    ...
    def __init__(self, config_path: str):
        ...
        # 初始化选股数据库服务 (用于保存到 daily_prediction 表)
        try:
            from services.db_pool import get_db_pool
            pool_manager = get_db_pool()
            self.selection_service = StockSelectionDBService(pool_manager)
        except Exception:
        ...
        self.logger = get_logger("tomorrow_picks.recommender")
    
    def _save_to_daily_prediction(self, filter_results: Dict[str, pl.DataFrame], pick_date: str):
        """保存筛选结果到 daily_prediction 表"""
        if not self.selection_service:
            self.logger.warning("选股服务未初始化，跳过保存到 daily_prediction")
            return
            ...
            for filter_type, df in filter_results.items():
                ...
                for row in records:
                    all_selections.append({
                        ...
                        'total_score': row.get('enhanced_score', row.get('score', 0)),
                        'close': row.get('price'),
                        'price_change': row.get('change_pct'),
                    })
            
            if all_selections:
                self.selection_service.save_to_daily_prediction(pick_date, all_selections, source='tomorrow_picks')
                self.logger.info(f"✅ 已保存 {len(all_selections)} 条记录到 daily_prediction 表")
        except Exception as e:
            self.logger.error(f"❌ 保存到 daily_prediction 失败: {e}")
    ...
    def run(self):
        ...
        try:
            ...
            if self.mysql_storage:
                ...
                # 同步保存到 daily_prediction 表
                self._save_to_daily_prediction(filter_results, pick_date)
...
if __name__ == '__main__':
    if FORCE_MODE:
        ...
        from pathlib import Path
        checker = DataFreshnessChecker(str(Path(__file__).parent.parent / "data"))
        if checker.check_data_freshness()[0]:
            main()
        else:
            print("⚠️ 数据不完整，使用 --force 跳过检查继续执行...")
            main()
    else:
        main()
