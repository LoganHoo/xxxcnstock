...
logger = get_logger("fund_behavior_strategy")
            ...
            if all_selections:
                ...
                for s in all_selections:
                    all_selections_for_daily.append({
                        'code': s.get('code', ''),
                        'name': s.get('name', ''),
                        'selection_type': s.get('selection_type', 'fund_behavior'),
                        'total_score': s.get('score', 0),
                        'close': s.get('close'),
                        'price_change': s.get('price_change'),
                    })
                selection_service.save_to_daily_prediction(report_date, all_selections_for_daily, source='fund_behavior')
...
def load_cctv_analysis(report_date: str = None) -> dict:
    ...
    try:
        ...
        db_host = os.getenv('MYSQL_HOST', os.getenv('DB_HOST'))
