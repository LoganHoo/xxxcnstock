...
class StockPicker:
    ...
    def __init__(self):
        ...
        self.logger = get_logger("stock_pick")
    ...
    def _save_picks(self, picks):
        ...
        try:
            ...
            all_selections = s_selections + a_selections
            if all_selections:
                try:
                    all_selections_for_daily = []
                    for s in all_selections:
                        all_selections_for_daily.append({
                            'code': s.get('code', ''),
                            'name': s.get('name', ''),
                            'selection_type': s.get('selection_type', 'stock_pick'),
                            'total_score': s.get('score', 0),
                            'close': s.get('close'),
                            'price_change': s.get('change_pct'),
                            'strategy_type': 'stock_pick',
                        })
                    self.db_service.save_to_daily_prediction(report_date, all_selections_for_daily, source='stock_pick')
