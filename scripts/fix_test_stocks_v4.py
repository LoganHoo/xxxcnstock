...
def fix_stock(code: str) -> bool:
    ...
    try:
        ...
        # 创建新数据行，匹配所有列
        # 计算 turnover (换手率) = volume / (总股本) - 这里用 baostock 返回的 turn
        # 计算 pct_chg (涨跌幅)
        new_data = {
            'trade_date': row[0],
            'code': code,
            ...
            'turnover': float(row[9]) if row[9] else 0.0,  # 换手率
            'pct_chg': float(row[10]) if row[10] else 0.0,  # 涨跌幅
