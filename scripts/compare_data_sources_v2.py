...
def get_baostock_data(code: str, query_date: str) -> dict:
    """从 baostock 获取数据"""
    lg = bs.login()
    if lg.error_code != '0':
        return None
...
def compare_stock(code: str):
    ...
    print(f"  本地最新日期: {local['date']}")
    
    # 使用本地数据的日期查询 baostock
    baostock = get_baostock_data(code, local['date'])
    
    if not baostock:
        print("  ❌ baostock 数据获取失败")
        return None
    
    print(f"  baostock日期: {baostock['date']}")
    print()
