...
class DataIntegrityChecker:
    ...
    def _get_db_config(self) -> Dict:
        ...
        if not host or not user:
            logger.warning("数据库配置不完整：DB_HOST、DB_USER 未设置，使用默认值")
    ...
    def _get_latest_data_date(self) -> str:
        ...
        try:
            ...
            latest_dates = []
            for file_path in parquet_files[:20]:  # 抽样20个文件
                try:
    ...
    def check_kline_data(self) -> CheckResult:
        ...
        try:
            ...
            try:
                ...
                for f in parquet_files[:100]:  # 抽样检查
                    try:
                        ...
                        date_col = 'trade_date' if 'trade_date' in df.columns else 'date'
                        if str(df[date_col].max()) == latest_data_date:
                            fresh_count += 1
                    except Exception:
                        continue
                fresh_rate = fresh_count / 100 if len(parquet_files) >= 100 else fresh_count / len(parquet_files)
            except Exception:
                fresh_rate = 0
            
            if count >= 4000 and fresh_rate >= 0.8:
                status = 'pass'
                message = f'✅ K线数据: {count}个文件，最新日期{latest_data_date}，新鲜度{fresh_rate:.0%}'
            elif count >= 4000:
                status = 'warning'
                message = f'⚠️ K线数据: {count}个文件，最新日期{latest_data_date}，新鲜度{fresh_rate:.0%}'
            else:
                status = 'fail'
            ...
            return CheckResult(
                ...
                min_required=4000,
                message=message,
                details={'latest_data_date': latest_data_date, 'fresh_rate': fresh_rate}
            )
