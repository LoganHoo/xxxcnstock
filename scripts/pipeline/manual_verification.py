...
try:
    ...
    logging.debug("python-dotenv 未安装，跳过环境变量加载")

# 使用环境变量或项目相对路径
DATA_DIR = Path(os.getenv("DATA_DIR", project_root / "data" / "kline"))
REPORT_DIR = Path(os.getenv("REPORT_DIR", project_root / "data" / "reports"))
TEMPLATE_DIR = Path(os.getenv("TEMPLATE_DIR", project_root / "templates"))
...
def check_data_freshness() -> Dict:
    ...
    for code in codes:
        try:
            ...
            if len(df) > 0:
                total_count += 1
                latest_date = df["trade_date"].max() if "trade_date" in df.columns else df["date"].max() if "date" in df.columns else None
