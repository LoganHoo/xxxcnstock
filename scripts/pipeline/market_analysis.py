...
def calculate_key_levels(df: pl.DataFrame) -> dict:
    ...
    try:
        if df.is_empty():
            logger.warning("数据为空，无法计算关键位")
            return {}
        
        recent = df.tail(60)
        latest = df.tail(1)
        
        if recent.is_empty() or latest.is_empty():
            logger.warning("数据不足，无法计算关键位")
            return {}
...
def calculate_cvd(df: pl.DataFrame, lookback: int = 20) -> dict:
    ...
    try:
        if df.is_empty():
            logger.warning("数据为空，无法计算CVD")
            return {'cvd_cumsum': 0, 'cvd_mean': 0, 'cvd_5d': 0, 'cvd_10d': 0, 'cvd_trend': 'neutral', 'signal': 'neutral'}
        
        df = df.tail(lookback + 10)
        
        if df.is_empty():
            logger.warning("数据不足，无法计算CVD")
            return {'cvd_cumsum': 0, 'cvd_mean': 0, 'cvd_5d': 0, 'cvd_10d': 0, 'cvd_trend': 'neutral', 'signal': 'neutral'}
...
def main():
    try:
        ...
        for code, name in indices:
            try:
                ...
                if result:
                    ...
                    logger.info(f'  收盘: {result["levels"]["close"]}')
                    logger.info(f'  均线: MA5={result["levels"]["ma5"]} MA20={result["levels"]["ma20"]} MA60={result["levels"]["ma60"]}')
                    logger.info(f'  压力位: R1={result["levels"]["resistance_1"]} R2={result["levels"]["resistance_2"]}')
                    logger.info(f'  支撑位: S1={result["levels"]["support_1"]} S2={result["levels"]["support_2"]}')
                    logger.info(f'  CVD: {result["cvd"]["signal"]} (趋势: {result["cvd"]["cvd_trend"]})')
                    logger.info(f'  结论: {result["analysis"]["conclusion"]} - {result["analysis"]["action"]}')
            except Exception as e:
                logger.error(f"分析指数 {code} 时出错: {e}")
                continue
        ...
        logger.info(f'  看多指数: {bullish_count}/{len(results)}')
        logger.info(f'  看空指数: {bearish_count}/{len(results)}')
        ...
        logger.info(f'  市场观点: {market_view}')
