#!/usr/bin/env python3
"""
使用 GE 1.16.1 新 API 设置 Great Expectations - 简化版
直接使用 Pandas 数据进行验证
"""
import sys
...
def setup_and_validate():
    ...
    # 1. 获取 DataContext - 使用 fluent API
    print("\n📂 初始化 DataContext...")
    ...
    print("✅ Datasource 创建成功")
    
    # 3. 创建 Data Asset
    print("\n📁 创建 Data Asset...")
    data_asset = datasource.add_dataframe_asset(name="stock_list_asset")
    print("✅ Data Asset 创建成功")
    
    # 4. 创建 Expectation Suite - 股票列表
    print("\n📋 创建股票列表期望套件...")
    ...
    try:
        ...
        if stock_list_path.exists():
            ...
            # 创建 Batch Definition
            batch_definition = data_asset.add_batch_definition_whole_dataframe("stock_list_batch")
            batch = batch_definition.get_batch(batch_parameters={"dataframe": df_pd})
            
            # 运行验证
            validator = context.get_validator(
                batch_request=batch.batch_request,
                expectation_suite=suite_stock
            )
    ...
    try:
        ...
        if kline_files:
            ...
            # 创建 Data Asset for Kline
            kline_asset = datasource.add_dataframe_asset(name="kline_asset")
            batch_definition = kline_asset.add_batch_definition_whole_dataframe("kline_batch")
            batch = batch_definition.get_batch(batch_parameters={"dataframe": df_pd})
            
            # 运行验证
            validator = context.get_validator(
                batch_request=batch.batch_request,
                expectation_suite=suite_kline
            )
            results = validator.validate()
