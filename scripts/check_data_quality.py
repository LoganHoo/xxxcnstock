...
def check_data_quality(kline_dir: Path, max_age_days: int = 30):
    """全面检查数据质量"""
    print("=" * 80)
    print("数据质量检查")
    print("=" * 80)
    ...
    for i, parquet_file in enumerate(parquet_files, 1):
        ...
        try:
            ...
            if len(df) == 0:
                empty_files += 1
                print(f"[{i}/{total_files}] {code}: 空文件")
                continue
...
def main():
    ...
    stale_stocks = check_data_quality(kline_dir)
    
    if stale_stocks:
        print(f"\n⚠️  检测到 {len(stale_stocks)} 只股票数据过旧")
        print(f"建议: 运行归档脚本处理这些文件")
    else:
        print(f"\n✅ 所有股票数据新鲜度正常")
