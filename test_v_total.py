...
print(f"Latest v_total (from factor): {latest['factor_v_total'][0]}")

# Calculate indicators
indicator = FundBehaviorIndicatorEngine()
indicators = indicator.calculate_all_indicators(data)

ms = indicators["market_sentiment"]
v_list = ms.get("avg_v_total", [])
print(f"avg_v_total count: {len(v_list)}")
print(f"Last avg_v_total: {v_list[-1] if v_list else 'empty'}")
...
print(f"\nStrategy result:")
print(f"  v_total from result: {result.get('v_total')}")
